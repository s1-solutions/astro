...
import type { HoistedScript } from '@astrojs/compiler/types';
...
export function vitePluginAnalyzer(
	...
	return {
		...
		async generateBundle() {
			const hoistScanner = createHoistedScriptScanner(options, internals, this);
			...
			for (const id of ids) {
				...
				await hoistScanner.scan(astro.scripts, id);
			...
			// commit gathered scripts into build internals
...
function createHoistedScriptScanner(
	options: StaticBuildOptions,
	internals: BuildInternals,
	pluginContext: PluginContext
) {
	const pageScripts: PageScripts = new Map

	return {
		async scan(scripts: HoistedScript[], from: string) {
			scanImpl(options, pluginContext, pageScripts, scripts, from)
		},
		finalize() {
			finalizeImpl(internals, pageScripts)
		}
	}
}
...
async function scanImpl(
	...
	if (hoistedScripts.size) {
		...
		for (const [parentInfo, depth] of walkParentInfos(from, pluginContext, isPropagatedAsset)) {
			...
			if (isPropagatedAsset(parentInfo.id)) {
				for (const [nestedParentInfo] of walkParentInfos(from, pluginContext)) {
